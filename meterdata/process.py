...
while True:
    try:
        ...
        # Execute the query
        query = "SELECT t.acmetersubsystemid, t.acmeterpolledtimestamp, CAST(t.acmeterenergy AS DECIMAL(18,2)), CAST(CASE WHEN t.acmeterenergy REGEXP '^[0-9]+\.?[0-9]*$' THEN t.acmeterenergy - t.latest_energy ELSE 0 END AS DECIMAL(18,2)), c.cumulative_diff, (SELECT SUM(cumulative_diff) FROM (SELECT acmetersubsystemid, MAX(acmeterpolledtimestamp) AS last_timestamp, SUM(CASE WHEN acmeterenergy REGEXP '^[0-9]+\.?[0-9]*$' THEN acmeterenergy - latest_energy ELSE 0 END) AS cumulative_diff FROM (SELECT acmetersubsystemid, acmeterpolledtimestamp, acmeterenergy, COALESCE(LAG(acmeterenergy) OVER (PARTITION BY acmetersubsystemid ORDER BY acmeterpolledtimestamp ASC), acmeterenergy) AS latest_energy FROM acmeterreadings WHERE acmetersubsystemid IN (1135, 1167, 350, 358) AND acmeterpolledtimestamp >= DATE_FORMAT(CURRENT_DATE(), '%Y-%m-%d 00:00:00') AND acmeterpolledtimestamp <= NOW()) t2 GROUP BY acmetersubsystemid) c2 WHERE acmetersubsystemid IN (1135, 1167, 350, 358)) as total_cumulative_diff FROM (SELECT acmetersubsystemid, acmeterpolledtimestamp, acmeterenergy, COALESCE(LAG(acmeterenergy) OVER (PARTITION BY acmetersubsystemid ORDER BY acmeterpolledtimestamp ASC), acmeterenergy) AS latest_energy FROM acmeterreadings WHERE acmetersubsystemid IN (1135, 1167, 350, 358) AND acmeterpolledtimestamp >= DATE_FORMAT(CURRENT_DATE(), '%Y-%m-%d 00:00:00') AND acmeterpolledtimestamp <= NOW()) t JOIN (SELECT acmetersubsystemid, MAX(acmeterpolledtimestamp) AS last_timestamp, SUM(CASE WHEN acmeterenergy REGEXP '^[0-9]+\.?[0-9]*$' THEN acmeterenergy - latest_energy ELSE 0 END) AS cumulative_diff FROM (SELECT acmetersubsystemid, acmeterpolledtimestamp, acmeterenergy, COALESCE(LAG(acmeterenergy) OVER (PARTITION BY acmetersubsystemid ORDER BY acmeterpolledtimestamp ASC), acmeterenergy) AS latest_energy FROM acmeterreadings WHERE acmetersubsystemid IN (1135, 1167, 350, 358) AND acmeterpolledtimestamp >= DATE_FORMAT(CURRENT_DATE(), '%Y-%m-%d 00:00:00') AND acmeterpolledtimestamp <= NOW()) t2 GROUP BY acmetersubsystemid) c ON t.acmetersubsystemid = c.acmetersubsystemid AND t.acmeterpolledtimestamp = c.last_timestamp;"
        ...
        for row in rows:
            ...
            insert_rows.append((device_id, polled_timestamp, energy_value, energy_diff, cumulative_energy,totalcummalativediff/1000))
            print(row)
